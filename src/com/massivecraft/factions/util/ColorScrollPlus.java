	...
	public String next() {
		...
		if (position >= str.length()) {
			...
			if (bold) {
				...
				String fin = textColor + "" + ChatColor.BOLD + str.substring(0, str.length() - 1) + colorBefore + ChatColor.BOLD + str.substring(str.length() - 1, str.length()) + two;
		...
		if (position == 0) {
			...
			if (bold) {
				String two = upperCaseMid ?
						colorMid + ChatColor.BOLD + one.toUpperCase() 
					  : colorMid + ChatColor.BOLD + one;
				String fin = two + colorAfter + ChatColor.BOLD + str.substring(1, 2) + textColor + ChatColor.BOLD + str.substring(2, str.length());
		...
		if (position >= 1) {
			...
			if (bold) {
				String three = upperCaseMid ?
						  colorMid + ChatColor.BOLD + str.substring(position, position + 1).toUpperCase()
						: colorMid + ChatColor.BOLD + str.substring(position, position + 1);
				String fin = null;
				...
				String first = m <= 1 ?
						  colorBefore + ChatColor.BOLD + one
						: ChatColor.BOLD + one.substring(0, one.length() - 1) + colorBefore + ChatColor.BOLD + one.substring(one.length() - 1, one.length());

				String second = l <= 1 ? 
						  colorAfter + ChatColor.BOLD + two
						: colorAfter + ChatColor.BOLD + two.substring(0, 1) + textColor + ChatColor.BOLD + two.substring(1, two.length());
				...
				String three = upperCaseMid ? 
						  colorMid + str.substring(position, position + 1).toUpperCase()
