...
{	
	...
	public static List<Location> getLocations(Player player, int steps, int step)
	{
		...
		if (steps < 1) throw new InvalidParameterException("steps devem ser maiores do que 0");
		if (step < 0) throw new InvalidParameterException("step deve pelo menos ser 0");
		if (step >= steps) throw new InvalidParameterException("step deve ser inferior a steps");
